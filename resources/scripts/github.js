// i have no clue how async/await fucking work 💀
...
async function fetchOriginalRepo(username, repo) {
    try {
...
function loadUserRepos(){
    fetchUserRepositories(ghn).then(repositories => {
        ...
        for(let i in repositories){
            if(repositories[i].fork){
                l[0]++
                fetchOriginalRepo(ghn, repositories[i].name).then(r => {
                    if(r!=null)
                        repositories[i] = r
                    l[0]--
                })
...
function waitForChange(valueToWatch, orig) {
    return new Promise(resolve => {
        const interval = setInterval(() => {
            if (valueToWatch[0] != orig) {
...
function waitForEq(valueToWatch, eq) {
    return new Promise(resolve => {
        const interval = setInterval(() => {
            if (valueToWatch[0] == eq) {
                clearInterval(interval);
                resolve(valueToWatch);
            }
        }, 100);
...
function createRepoList() {
    waitForChange(repos, null).then(() => {
        const c = document.getElementById("github")
        c.innerHTML = ""
        for(let i in repos[0]){
            const e = document.createElement("div")
            e.classList.add("entry")

            let gen = document.createElement("div")
            let name = document.createElement("a")
            name.innerText = repos[0][i].name
            name.href = repos[0][i].html_url
            gen.appendChild(name)
            let desc = document.createElement("p")
            desc.innerText = repos[0][i].description
            ...
            if(repos[0][i].fork){
                ...
                if(repos[0][i].source!=undefined){
                    fork.append(repos[0][i].source.owner.login)
                    fork.href = repos[0][i].source.html_url
                }
            ...
            stars.append(repos[0][i].stargazers_count)
            stats.appendChild(stars)
            let forks = document.createElement("div")
            forks.innerHTML = '<svg aria-hidden="true" viewBox="0 0 16 16" version="1.1" data-view-component="true" class="octicon octicon-repo-forked mr-2"><path d="M5 5.372v.878c0 .414.336.75.75.75h4.5a.75.75 0 0 0 .75-.75v-.878a2.25 2.25 0 1 1 1.5 0v.878a2.25 2.25 0 0 1-2.25 2.25h-1.5v2.128a2.251 2.251 0 1 1-1.5 0V8.5h-1.5A2.25 2.25 0 0 1 3.5 6.25v-.878a2.25 2.25 0 1 1 1.5 0ZM5 3.25a.75.75 0 1 0-1.5 0 .75.75 0 0 0 1.5 0Zm6.75.75a.75.75 0 1 0 0-1.5.75.75 0 0 0 0 1.5Zm-3 8.75a.75.75 0 1 0-1.5 0 .75.75 0 0 0 1.5 0Z"></path></svg>'
            forks.append(repos[0][i].forks_count)
